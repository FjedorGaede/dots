# commit.sh — `dots commit`: open lazygit in the dotfiles repo.
# Manual commits belong in a real TUI (staging, messages, push — lazygit does
# it all). The *automatic* part lives in add/remove via repo_commit_paths
# (common.sh): tracking changes commit themselves surgically and push.
